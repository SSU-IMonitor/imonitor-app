...
- (void)onCalibrationNextPointWithX:(float)x y:(float)y;
- (void)onCalibrationProgressWithProgress:(float)progress;
...
SWIFT_PROTOCOL("_TtP5SeeSo19EyeMovementDelegate_")
@protocol EyeMovementDelegate <GazeTrackerDelegate>
- (void)onEyeMovementWithTimestamp:(double)timestamp duration:(double)duration x:(float)x y:(float)y state:(enum EyeMovementState)state;
@end
...
- (void)onGazeWithTimestamp:(double)timestamp x:(float)x y:(float)y state:(enum TrackingState)state;
- (void)onFilteredGazeWithTimestamp:(double)timestamp x:(float)x y:(float)y state:(enum TrackingState)state;
@end
...
@property (nonatomic, weak) id <EyeMovementDelegate> _Nullable eyeMovementDelegate;
...
- (BOOL)startCalibrationWithMode:(enum CalibrationMode)mode region:(CGRect)region SWIFT_WARN_UNUSED_RESULT;
...
- (void)setDelegatesWithStatusDelegate:(id <StatusDelegate> _Nullable)statusDelegate gazeDelegate:(id <GazeDelegate> _Nullable)gazeDelegate calibrationDelegate:(id <CalibrationDelegate> _Nullable)calibrationDelegate eyeMovementDelegate:(id <EyeMovementDelegate> _Nullable)eyeMovementDelegate imageDelegate:(id <ImageDelegate> _Nullable)imageDelegate;
/// Get SDK version
...
typedef SWIFT_ENUM(NSInteger, InitializationError, closed) {
  ...
  InitializationErrorERROR_AUTHENTICATE = 3,
};
...
typedef SWIFT_ENUM(NSInteger, TrackingState, closed) {
  ...
  TrackingStateCALIBRATING = 1,
  TrackingStateFACE_MISSING = 2,
  TrackingStateOUT_OF_SCREEN = 3,
};
